...
test('la portada muestra un ejemplo sin reemplazar la tarjeta del visitante', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByTestId('ejemplo-con-foto').getByRole('heading', { name: 'Alexa Rivera' })).toBeVisible()
  await expect(page.getByTestId('ejemplo-con-foto').getByRole('img', { name: 'Foto de Alexa Rivera' })).toHaveAttribute('src', '/ejemplos/profesional-ia.webp')
  await expect(page.getByTestId('ejemplo-sin-foto').getByRole('img', { name: 'Foto de Alexa Rivera' })).toHaveCount(0)
  await expect(page.getByTestId('ejemplo-contacto')).toContainText('alexa@example.com')
